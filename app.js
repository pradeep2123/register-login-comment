...
app.get('/confirmation/:token', (req,res,next)=>{
 ...
 // Verify and save the user
        ...
        user.save(function (err){ 
            ...
            if (err) 
            { 
              return res.render(path.join(__dirname,'/views/error.ejs'),{message: "Not saved to database "});
            }
            else
            {
        ...
        console.log("is it coming to password")
           ...
           if(user)
              ...
              if (bcrypt.compareSync(password,user.password) && (user.active == true)) 
                ...
                res.render(path.join(__dirname,'/views/error.ejs'),{message:"not a authorization user, go and verifiy a link"}) 
